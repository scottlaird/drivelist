...
var derivedEvents = []string{
	EventFirstSeen, EventAppeared, EventVanished, EventReappeared, EventMovedHost, EventMovedBay,
...
func (t *tx) snapshotRows(snapshotID int64) ([]devRow, error) {
	rows, err := t.QueryContext(t.ctx, `SELECT dev_name, drive_id, expander, bay, enclosure_path, size_bytes, uses, member_state, dev_links, scsi_addr, error FROM snapshot_device WHERE snapshot_id = ? ORDER BY dev_name`, snapshotID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&d.DevName, &driveID, &d.Expander, &d.Bay, &d.EnclosurePath, &d.SizeBytes, &uses, &d.MemberState, &links, &d.SCSIAddr, &d.Error); err != nil {
			return nil, err
		}
